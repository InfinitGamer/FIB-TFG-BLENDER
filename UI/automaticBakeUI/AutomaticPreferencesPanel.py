...
class AutomaticPreferencesPanel(bpy.types.Panel):
    ...
    def get_button(scene: bpy.types.Scene,
       ...
       button =  row.operator("ui.set_automatic_settings",text=text_button,
                              icon='RADIOBUT_ON' if scene.UIbake_settings.automatic_setting_number == button_number else 'RADIOBUT_OFF')
       button.width = width
    ...
    def draw(self, context):
        ...
        AutomaticPreferencesPanel.get_button(scene=scene,
                                             row=row,
                                             width=512,
                                             height=512,
                                             margin = 16,
